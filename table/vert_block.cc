...
VertSection::VertSection(bool reading) : num_entry_(0), estimated_size_(0),
                                         reading_(reading) {
}

VertSection::VertSection(Encodings enc) : VertSection(false) {
    value_enc_ = enc;
    value_encoder_ = EncodingFactory::Get(value_enc_).encoder();
}
...
void VertSection::Read(const char *in) {
    ...
    // Read encoding type
    value_enc_ = static_cast<Encodings>(*(pointer++));
    auto encoding_ = EncodingFactory::Get(static_cast<Encodings>(value_enc_));
    value_decoder_ = encoding_.decoder();
