...
namespace leveldb {
    ...
    Block::Block(const BlockContents &contents) {
        // Read the last byte and determine what is in the block
    ...
    inline uint32_t BasicBlockCore::NumRestarts() const {
        assert(size_ >= sizeof(uint32_t));
        return DecodeFixed32(data_ + size_ - sizeof(uint32_t));
    }

    BasicBlockCore::BasicBlockCore(const BlockContents &contents)
...
// pointer to the key delta (just past the three decoded values).
    ...
    class BasicBlockCore::Iter : public Iterator {
    private:
    ...
    Iterator *BasicBlockCore::NewIterator(const Comparator *comparator) {
        if (size_ < sizeof(uint32_t)) {
